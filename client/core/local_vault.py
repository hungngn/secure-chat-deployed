...
class LocalVault:
    ...
    def _save(self, table, pk_value, data_dict):
        ...
        self.conn.execute(f"INSERT OR REPLACE INTO {table} ({pk_col}, {col_data}) VALUES (?, ?)", (pk_value, json.dumps(data_dict)))
        self.conn.commit()
    ...
    def save_message(self, friend_id, message, is_me):
        self.conn.execute("INSERT INTO chat_history (friend_id, message, is_me) VALUES (?, ?, ?)", (friend_id, message, 1 if is_me else 0))
        self.conn.commit()
